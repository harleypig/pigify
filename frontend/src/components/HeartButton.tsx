...
import { apiService, type FavoriteWriteBody } from "../services/api";
...
function HeartButton({
  ...
  useEffect(() => {
    if (initialSpotifyLoved !== undefined && initialLastfmLoved !== undefined)
      return;
    ...
    apiService
      .checkFavorites([
        {
          track_id: track.spotify_id ?? undefined,
          name: track.name,
          artist: track.artist,
        },
      ])
  ...
  const title = (() => {
    const parts: string[] = [];
    parts.push(
      `Spotify: ${spotifyLoved === null ? "?" : spotifyLoved ? "loved" : "not loved"}`,
    );
    parts.push(
      `Last.fm: ${lastfmLoved === null ? "n/a" : lastfmLoved ? "loved" : "not loved"}`,
    );
