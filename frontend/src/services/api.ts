...
export const apiService = {
  ...
  async undoReorder(
    playlistId: string,
  ): Promise<{ restored: boolean; tracks: number }> {
  ...
  async getUndoStatus(
    ...
    const response = await apiClient.get(
      `/api/playlists/${playlistId}/undo-status`,
    );
    return response.data;
  },
  ...
  async playTrack(trackUri?: string, deviceId?: string): Promise<void> {
    await apiClient.put("/api/player/play", {
  ...
  async seekTo(positionMs: number): Promise<void> {
    await apiClient.put(
      `/api/player/seek?position_ms=${Math.round(positionMs)}`,
    );
  },
  ...
  async getAudioAnalysis(
    ...
    const response = await apiClient.get(`/api/player/analysis/${trackId}`, {
      params: { bars },
    });
    return response.data;
  },
  ...
  async clearLastfmQueue(ids?: number[]): Promise<LastfmQueueClearResult> {
  ...
  async syncFavorites(maxTracks = 500): Promise<SyncSummary> {
    const r = await apiClient.post("/api/favorites/sync", {
  ...
  async resolveFavoriteConflict(
    ...
    const r = await apiClient.post("/api/favorites/resolve-conflict", {
      index,
      choice,
    });
    return r.data;
...
export const recipesApi = {
  ...
  async play(id: string, uris?: string[]): Promise<RecipePlayResponse> {
    const r = await apiClient.post(`/api/recipes/${id}/play`, {
      uris: uris ?? null,
    });
    return r.data;
  },
